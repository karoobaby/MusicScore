...
public class Udpthread implements Runnable{
	...
	private Handler vhandler = new Handler()
	{
		...
		public void handleMessage(Message msg)
		{
				...
				if (getsRecvData()==null) return;
				StringBuilder sb = new StringBuilder();
				sb.append(getsRecvData());
				sRecvStr=sb.toString().trim();
	...
	public Udpthread(String sRecvStr)
	{
		this.sRecvStr=sRecvStr;
	}
	...
	private void recvData()
	{
		try
		{
			...
			if (isRHex)
				...
				setsRecvData(new String(rPacket.getData(),0,rPacket.getData().length,"utf-8")); 
			else
				setsRecvData(new String(rPacket.getData(),currentRCodes).trim());
			setsRecvData(String.format("[%s:%d//%s]%s", rPacket.getAddress().getHostAddress(), rPacket.getPort(), sdf.format(new Date()), getsRecvData()));
	...
	private void setsRecvData(String sRecvData) {
		this.sRecvData = sRecvData;
	}
